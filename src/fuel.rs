...
impl Fuel {
    ...
    ///
    /// It credits the running continuation with fuel while also preventing available fuel from
    /// growing to an unbounded level, and also clears the interrupt flag since we are now on the
    /// "outside" of whatever lua code is being run and this is where we would want to interrupt to.
