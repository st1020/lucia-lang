...
///
/// Like [`Table`]s, `UserData` may have "metatables" which allow Lucia code to trigger special
/// functions (metamethods) when Lucia operations are performed on them.
///
/// `UserData` in Lucia fully integrate with the garbage collector and may be any (custom) garbage
/// collected type. Downcasting and mutating garbage collected types is however more complicated
/// than normal non-garbage collected types, so there are two parallel APIs provided here: one for
/// any garbage collected type and one only for `'static` types that is provided as a convenience
/// over the more general interface for arbitrary GC types.
///
